    ...
    <Router>
          ...
          <div className={`nav-links ${isMenuOpen ? 'open' : ''}`}>
            {user && (
              <>
                ...
                <Link to="/my-vault" className="nav-item" onClick={() => setIsMenuOpen(false)}>My Quizzes</Link>
              </>
            )}
                    ...
                    {showNotifications && (
                        <div className="notifications-dropdown" style={{ position: 'absolute', top: '100%', right: 0, background: 'var(--bg-secondary)', border: '1px solid var(--border-dark)', padding: '1rem', width: '250px', zIndex: 100, borderRadius: '8px' }}>
                            ...
                            {notifications.length === 0 ? <p style={{ fontSize: '0.9rem' }}>No new messages.</p> : (
                                notifications.map(n => (
                                    <div key={n.id} style={{ padding: '8px', borderBottom: '1px solid var(--border-dark)', fontSize: '0.85rem', color: n.is_read ? 'var(--text-muted)' : 'var(--text-main)' }} onClick={async () => {
                                        if(!n.is_read){
      ...
      <div className="main-content">
        <Routes>
          ...
          <Route path="/create-module" element={<ProtectedRoute user={user}><CreateModule /></ProtectedRoute>} />
